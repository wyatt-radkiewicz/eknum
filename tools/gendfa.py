...
class States:
    ...
    def load_xml(self, am: Element):
        ...
        self.reorder = [0] * (len(norm) + len(done) + 1)
        newid = 0
    ...
    def print_cinfo(self):
        for s in self.accept_states:
            name = (FILE_NAME + '_' + s[1]).upper()
            print(f'#define {name} {s[0]}')
        for s in self.done_states:
            name = (FILE_NAME + '_' + s[1]).upper()
            print(f'#define {name} {s[0]}')
        name = (FILE_NAME + '_ERR').upper()
...
class Transitions:
    ...
    def print_ctable(self):
        print('// Edge table')
        print(f'static const uint8_t {FILE_NAME}_edges[256] = {{')
        for i in range(0, 16):
            print('\t', end='')
            for j in range(0, 16):
        ...
        print(f'static const uint8_t {FILE_NAME}_trans_compact'
        ...
        print(f'static const uint8_t {FILE_NAME}_trans[][256] = {{')
        for i in range(0, len(self.states)):
            for j in range (0, 16):
                if j != 0: print('\t  ', end='')
                else: print('\t{ ', end='')
